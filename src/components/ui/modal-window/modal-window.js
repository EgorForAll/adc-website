  ...
  return (
    <div className="modal-window" ref={modalRef}>
      ...
      <form action="mail.php" className="modal-window__form" ref={formRef} method="POST">
        ...
        <h4 className="modal__window-subtitle--services" htmlFor="type">Выберите услугу:</h4>
          {catalogList.map((item) =>
            <div className="modal-window__services-wrapper">
             <input type="checkbox" name={item.name} id={item.id} className="modal-window__input--checkbox"/>
            <label htmlFor={item.name} className="modal__window-subtitle--checkbox">{item.name}</label>
            </div>
          )}
